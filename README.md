...
- Реализация тестового задания для команды DimaTech.
- Использовал FastAPI с использованием Swagger UI.
- Добавлены инструкции для деплоя как в Docker Compose так и локально.
- Файл с настройками проекта (.env) был добавлен в публичный репозиторий НАМЕРЕННО, что бы быстро проверить работоспособность проекта.
- Заполнения тестовыми данными реализованы реализованы через незащищенный эндпоинт /seed/test-data.
...
(Вместо 8888 ставим порт на котором запущен сервер с приложением)
